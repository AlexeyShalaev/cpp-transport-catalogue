...
namespace transport::response {
    ...
    RequestHelper::RequestHelper(TransportCatalogue &tc, const json::Array &stat_requests) : catalogue_(tc) {
            ...
            switch (request.type) {
                ...
                case RequestType::ROUTER: {
                    if (auto graph_router = catalogue_.findRouteInBase(request.from, request.to); graph_router) {
                        responses_.push_back(CreateJsonResponseRoute(request.id, graph_router));
                    } else {
                        responses_.push_back(CreateJsonResponseError(request.id));
                    }
    ...
    json::Node RequestHelper::CreateJsonResponseBus(const int request_id, const domains::Bus &data) {
        return json::Builder{}.
                ...
                Key("stop_count"s).Value(static_cast<int>(data.stop_count)).
                Key("unique_stop_count"s).Value(static_cast<int>(data.unique_stop_count)).
    ...
    json::Node
    RequestHelper::CreateJsonResponseRoute(const int request_id, const std::shared_ptr<std::vector<RouteItem>> &route) {
        ...
        for (auto &it: *route) {
            ...
            builder_.StartDict()
                    ...
                    .Key("span_count"s).Value(static_cast<int>(it.stop_count))
