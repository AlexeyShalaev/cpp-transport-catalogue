...
namespace transport::catalogue {
    ...
    TransportCatalogue::TransportCatalogue(
        ...
        for (auto &bus: buses) {
            ...
            bus->unique_stop_count = static_cast<int>(stops_uniq.size());
            ...
            for (size_t i = 0; i + 1 < bus->stops.size(); i++) {
                ...
                bus->route_length += RealLenBetweenStops(stops_[bus->stops[i]], stops_[bus->stops[i + 1]]);
            }


            if (bus->is_round_trip) {
                bus->stop_count = static_cast<int>(bus->stops.size());
            } else {
                bus->stop_count = static_cast<int>(bus->stops.size()) * 2 - 1;
                len_by_coordinates *= 2;

                for (int i = static_cast<int>(bus->stops.size()) - 1; i - 1 > -1; i--) {
                    bus->route_length += RealLenBetweenStops(stops_[bus->stops[i]], stops_[bus->stops[i - 1]]);
                }
            }
    ...
    TransportCatalogue::findRouteInBase(std::string_view from, std::string_view to) {
    ...
    std::string TransportCatalogue::Serialize() const {
        ...
        for (const auto &[_, stop]: stops_) {
            ...
            for (const auto &[name, len]: stop->road_distance) {
                auto &road = *proto_stop.add_road_distance();
                road.set_name(name);
                road.set_len(len);
            }
        }
        ...
        for (const auto &[name, bus]: buses_) {
            ...
            proto_bus.set_is_round_trip(bus->is_round_trip);
    ...
    void TransportCatalogue::Deserialize(const std::string &data) {
        ...
        for (const TCProto::Stop &proto_stop: proto.map_stops()) {
            ...
            for (const auto &road: proto_stop.road_distance()) {
                stop.road_distance[road.name()] = static_cast<int>(road.len());
            }
        ...
        for (const TCProto::Bus &proto_bus: proto.map_buses()) {
            ...
            bus.is_round_trip = proto_bus.is_round_trip();
        ...
        render_ = std::make_unique<MapRenderer>(stops_, buses_,
                                                MapRenderer::DeserializeSettings(proto.renderer_settings()));
        map_ = proto.catalogue_map();

        router_ = std::make_unique<TransportRouter>(stops_, buses_,
                                                    TransportRouter::DeserializeSettings(proto.routing_settings()));
